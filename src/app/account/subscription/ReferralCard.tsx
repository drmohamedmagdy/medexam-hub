...
export default function ReferralCard({
  ...
  function shareWhatsapp() {
    const text = encodeURIComponent(t.referralWhatsappShareText.replace("{url}", referralUrl));
  ...
  return (
    <section className="mt-8 rounded-2xl border border-emerald-200 bg-gradient-to-br from-emerald-50 to-teal-50 p-5 dark:border-emerald-900 dark:from-emerald-950/30 dark:to-teal-950/30 sm:p-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 className="text-lg font-semibold">{t.referralHeading}</h2>
          <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-300">{t.referralBody}</p>
        </div>
        <div className="text-end">
          <div className="text-xs font-medium uppercase tracking-wide text-zinc-500">
            {t.referralBalanceLabel}
          </div>
      ...
      <ul className="mt-5 grid gap-2 text-xs text-zinc-600 dark:text-zinc-300 sm:grid-cols-3">
        <li className="flex items-start gap-2">
          ...
          <span>{t.referralCommissionBasic}</span>
        </li>
        <li className="flex items-start gap-2">
          <span className="text-emerald-600 dark:text-emerald-400">+100</span>
          <span>{t.referralCommissionPro}</span>
        </li>
        <li className="flex items-start gap-2">
          <span className="text-emerald-600 dark:text-emerald-400">+200</span>
          <span>{t.referralCommissionPremium}</span>
        </li>
      </ul>
      <p className="mt-3 text-xs text-zinc-500">{t.referralRedemptionHint}</p>
    </section>
  );
}
